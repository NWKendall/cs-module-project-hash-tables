...
class HashTable:
    ...
    def put(self, key, value):
        ...
        print("Node:", node)
        while node is not None & node.key is not key:   
            node = node.next
        if node is not None:
            node.value = value
        else:
            new_node = Node(key, value)
            new_node.next = self.buckets[node]
            self.buckets[node] = new_node
            self.size += 1
            # check for resize criteria
    ...
    def resize(self, new_capacity):
        ...
        #         # if multiple nodes exist in LL
        #         while cur is not None:
        #             print("XXX", node_index)
